...
intrinsics = np.load(os.path.join(dirname,'../data/intrinsics.npz'))
K1, K2 = intrinsics['K1'], intrinsics['K2']

extrinsics = np.load(os.path.join(dirname,'../data/extrinsics.npz'))
...
np.savez(os.path.join(dirname,'../data/rectify.npz'), M1=M1, M2=M2, K1p=K1p, K2p=K2p, R1p=R1p, R2p=R2p, t1p=t1p, t2p=t2p)
...
corresp = np.load(os.path.join(dirname,'../data/some_corresp.npz'))
